...
class TIC_TAC_TOE_AI:
    def __init__(self, root):
        ...
        self.make_canvas = Canvas(
            self.window, background="#141414", relief=RAISED, bd=3)
        ...
        self.surrounding_store = {1: (2, 3, 4, 7), 2: (1, 3), 3: (1, 2, 6, 9), 4: (
            1, 7), 5: (2, 4, 6, 8), 6: (3, 9), 7: (1, 4, 8, 9), 8: (7, 9), 9: (7, 8, 6, 3)}

        self.decorating()

    def decorating(self):  # Basic Set-up
        Label(self.make_canvas, text="Tic-Tac-Toe AI\nAn Infinity Production",
              bg="#141414", fg="#00FF00", font=("Lato", 25, "bold")).place(x=110, y=10)
    ...
    # Control give based on human first or computer first play
    def control_give(self, indicator="human_first"):
        self.machine_first_control.config(
            state=DISABLED, disabledforeground="grey")
        self.human_first_control.config(
            state=DISABLED, disabledforeground="grey")
    ...
    def __sign_insert(self, btn_indicator, sign_is="X"):  # Button sign Insert
        if sign_is == "X":
            self.activate_btn[btn_indicator - 1].config(
                text=sign_is, state=DISABLED, disabledforeground="#00FF00")
        else:
            self.activate_btn[btn_indicator - 1].config(
                text=sign_is, state=DISABLED, disabledforeground="red")
                            ...
                            if human_last == 8:
                                ...
                                if take_center_surr:
                                    if (human_last+3 == 7 or human_last+3 == 9) or human_last+3 in self.sign_store.keys():
                                        ...
                                        take_place = random.choice(
                                            take_center_surr)
                                        take_center_surr.remove(take_place)
                                    self.__sign_insert(take_place)
                                    self.machine_cover.append(take_place)
                                    self.surrounding_store[5] = tuple(
                                        take_center_surr)
                                    self.prob.append(opposite[take_place])
                                else:
            ...
            if self.technique == 1:
                ...
                if self.technique == 3.2:
                    ...
                    if human_input in self.prob:
                        ...
                        self.prob.append(
                            next_human_prob[machine_next_chance[human_input]][0])
                        self.prob.append(
                            next_human_prob[machine_next_chance[human_input]][1])
                    else:
                        self.__sign_insert(self.prob[0])
                        self.machine_line_match()
